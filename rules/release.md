...
> ⚠️ **预发布标识必须是纯数字**：Windows 的 MSI 目标（WiX `ProductVersion`）不接受字母预发布
> 标识，写成 `1.1.0-rc.1` 会在 `tauri build --bundles msi` 阶段直接失败（报错原文：
...
| 阶段         | 版本号    | 说明                 | 门控                     |
| ------------ | --------- | -------------------- | ------------------------ |
| 预发布 Alpha | `1.1.0-N` | 内部测试，功能不完整 | 无                       |
| 预发布 Beta  | `1.1.0-N` | 功能完整，可能有 Bug | E1/E5 门控通过           |
| 预发布 RC    | `1.1.0-N` | 候选发布，仅修 Bug   | E11 门控通过             |
| 正式 Stable  | `1.1.0`   | 正式发布             | 全量测试通过 + 无 P0 Bug |
