...
| 阶段 | 版本号 | 说明 | 门控 |
|------|--------|------|------|
| Alpha | `0.x.0-alpha.N` | 内部测试，功能不完整 | 无 |
| Beta | `0.x.0-beta.N` | 功能完整，可能有 Bug | E1/E5 门控通过 |
| RC | `0.x.0-rc.N` | 候选发布，仅修 Bug | E11 门控通过 |
| Stable | `0.x.0` | 正式发布 | 全量测试通过 + 无 P0 Bug |

### 版本号变更规则

| 变更类型 | 版本号变更 | 示例 |
|----------|------------|------|
| 新 Epic 完成 | MINOR +1 | `0.1.0` → `0.2.0` |
| Bug 修复 | PATCH +1 | `0.1.0` → `0.1.1` |
| IPC 接口重构 | MAJOR +1 | `0.1.0` → `1.0.0` |
| 预发布 | 后缀 | `0.2.0-beta.1` |
...
```
1. 创建 release 分支: git checkout -b release/0.2.0
2. 更新版本号:
   - package.json: "version": "0.2.0"
   - src-tauri/Cargo.toml: version = "0.2.0"
   - src-tauri/tauri.conf.json: "version": "0.2.0"
   - python-sidecar/app/main.py: version="0.2.0"
3. 更新 Changelog: [Unreleased] → [0.2.0] - YYYY-MM-DD
4. 全量测试: make test
5. 三平台构建: make build
6. 签名: macOS 公证 + Windows OV 签名
7. 创建 GitHub Release: tag v0.2.0 + 上传安装包
...
```bash
# 创建 tag
git tag -a v0.2.0 -m "Release 0.2.0: 三层分类引擎 + RAG 问答"
