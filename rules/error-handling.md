...
| EMB-V-001  | Embedding | "离线模型包内容不符合要求"                 | 包内无可用模型 / 模型目录缺文件（T1 离线导入）                                   | 422      |
| EMB-U-002  | Embedding | "模型导入失败"                             | 离线包不可读（zip 损坏 / 路径失效）/ 落盘失败（T1 离线导入）                     | 500      |
| SEC-P-001  | 安全      | "操作需要确认"                             | 模式切换需同意                                                                   | 403      |
| KEY-U-001  | 密钥存储  | "系统密钥链访问失败，请检查钥匙串是否锁定" | Keychain 读写失败（BE-m1，Rust 侧 `AppError::Keychain`，命令层 KEY-001/002/003） | 500      |
