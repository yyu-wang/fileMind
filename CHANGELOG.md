...
- **T3.2** 分类预览与执行：`preview_operations` 重写（batch\_id + plan + summary + 4 冲突策略）、移动/复制、批量撤销。
...
- **T10.2** RAG 首 token 优化：查询缓存 + Ollama keep\_alive/num\_ctx + httpx/LanceDB 连接复用 + TTFT 埋点。

- **T10.5** 基准脚本：`benchmarks/`（scan\_perf + rag\_bench + run-all 一键报告）。
