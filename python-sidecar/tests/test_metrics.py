...
2. **已签名调用** → 200，``rss_mb / vms_mb`` 为正数 float、``threshold_mb=2048``、
   ``within_limit`` 为 bool（测试环境必为 True，因为冷启动远低于 2048MB）。
...
def test_metrics_signed_success(client: TestClient) -> None:
    ...
    data = resp.json()
    # 类型校验：rss/vms 为正数 float，threshold 为 2048，within_limit 必 True
    ...
    assert data["threshold_mb"] == 2048
    assert isinstance(data["within_limit"], bool)
    # 冷启动必然低于 2048MB；若此处 False 说明测试环境异常（需人工排查）
    assert data["within_limit"], "冷启动内存应低于 2048MB"
    # 响应中不得泄漏 PSK
...
def test_metrics_threshold_env_override(
    ...
    """env FILEMIND_MEMORY_THRESHOLD_MB 覆盖门控阈值；非法值回落默认 2048。
