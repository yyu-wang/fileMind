...
> **开发任何 UI 组件前，必须先查阅 `项目开发文件/05_交互原型.html` 中对应页面的交互效果。**

原型是交互行为的唯一基准，以下要素必须与原型保持一致：

| 约束维度 | 原型定义                                                                                | 代码实现要求                                              |
| -------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| 设计令牌 | CSS 变量：颜色 `--accent`/`--warn`/`--purple`、间距、圆角 `--radius`、阴影 `--shadow-*` | 在 `src/styles/` 中定义同名变量，组件引用而非硬编码       |
| 字体     | `--font-sans`（Noto Sans SC）、`--font-mono`（JetBrains Mono）                          | 全局 CSS 中定义，组件不覆盖                               |
| 动画曲线 | `--ease`、`--ease-out`、`--ease-spring`                                                 | transition/animation 中使用变量名，不直接写 cubic-bezier  |
| 页面动画 | `pageIn`、`fadeIn`、`scaleIn` 关键帧                                                    | 页面切换、面板入场时复用同名 keyframes                    |
| 组件交互 | Modal、Toast（`toastIn`/`toastOut`）、Drawer 侧滑、Tab 切换、Tree 展开                  | 组件动画时长、easing、方向与原型一致                      |
| 聊天动画 | 消息入场 `msgIn`、打字光标 `blink`                                                      | Chat 页面消息列表和输入框必须实现                         |
| 引导流程 | Logo 入场 `logoIn`、步骤入场 `stepIn`                                                   | 首次启动引导页必须实现                                    |
| 暗色模式 | `[data-theme="dark"]` 全套变量映射                                                      | 所有颜色/阴影引用变量，切换 `data-theme` 属性即可整体适配 |

**禁止行为**：

- 不得擅自修改原型中定义的 transition 时长和 easing 曲线
- 不得在代码中使用原型中未定义的动画效果（如需新增，须先在原型中补充定义）
- 不得硬编码颜色值，必须使用 CSS 变量
